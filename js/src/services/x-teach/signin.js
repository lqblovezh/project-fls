 ...
 * @param {String} data.back_url - 必选 返回址 成功时会带上 token_id=xx,失败会带上 error=xxx
 */
 ...
 * 密码找回-发短信
 * @param {Object} data
 * @param {String} data.user_name - 用户
 * @param {String} data.code_id - 图片id
 * @param {String} data.code_val - 图片值
 */
export function pwdBackPhoneCode(data = {}) {
  return request_post('/common/api/user/pass_reset/send', data)
}
/**
 * 密码找回-reset pwd
 * @param {Object} data
 * @param {String} data.user_id - 用户 用户ID （找回密码发送验证码的接口返回的）
 * @param {String} data.pass - 密码
 * @param {String} data.pass_confirm - 确认密码
 * @param {String} data.code_val - 用户收到的验证码
 */
export function pwdBackRestPwd(data = {}) {
  return request_post('/common/api/user/pass_reset/send', data)
}
...
export function changePass(data = {}) {
  return request_post('/common/api/auth/pass', data).then(res => {
    window.sessionStorage.removeItem('token')
    window.sessionStorage.removeItem('userInfo')
    config.token = null
    return res
