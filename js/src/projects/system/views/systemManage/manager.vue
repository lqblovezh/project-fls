...
export default {
  ...
  methods: {
    ...
    getRoleInfo(pageNum = 1, pageOffset = 1000,) {
      managerServices.listRole({pageNum, pageOffset,app_type_all:1}).then(res => {
        // log(res.data)
        // log(res.data[1].app_type)
    ...
    showDetaile(id) {
      ...
      if (!id) {
        ...
        if(roleInfo.roles[0]){
          this.detailDate.role = roleInfo.roles[0].name
        }
        this.detailVisible = true
      }
    },
    ...
    async save() {
      let ro = this.roleInfo.roleList.find(item => item.name == this.detailDate.role)
    ...
    table.table
      ...
      tbody(v-if="pageList.length")
        tr(v-for="item in pageList")
          ...
          td
            span(v-for="(items,key) in item.roles") {{items.name}}
          td
  ...
  width="600px"
    ...
    el-form(:model="detailDate" :rules="detailRules" ref="detailDialog")
      ...
      el-form-item(label="角色:" label-width="120px" prop="role")
        ...
        //-     :value="item.id"
        //-   )
        el-radio-group(v-model="detailDate.role"
          :min="1"
          :max="100")
          el-radio(v-for="item in roleInfo.roleList" :label="item.name" :key="item.id" name="role") {{item.name}}
