<script>
import * as services from 'services/system/demo'
import {
  getTask
  ...
  methods: {
    ...
    async sendMessage(file){
      ...
      if(file2){
        ...
        this.form.img_code = res.data.path
        })
      }
...
div
  ...
  el-table(ref="table"
    ...
    el-table-column(label="二维码")
      template(slot-scope="scope")
        .pic(:style="{backgroundImage:'url(' + scope.row.img_code_src + ')'}")
    el-table-column( prop="nick_name" label="文字介绍" width="600")
      template(slot-scope="scope")
        pre {{scope.row.text}}
  ...
  el-dialog(:visible.sync="dialogVisible" width="800px" :before-close="handleClose" @opened="clear")
    .box
      .part
        .img
      ...
      .part
        .img
          img(:src="form.img_code_src" ref="img2")
        input.hide(type="file" @change="upload($event,'img2')" ref="updateImg2")
        el-button(type="primary" size="small" @click="$refs.updateImg2.click()" plain) 二维码上传
