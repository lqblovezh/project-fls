...
import { canRead, noLoginToast, themeIsBuy, payTypeConfig } from '@/utils/util.js'
...
export default {
  ...
  computed: {
    ...
    isBuy() {
      return themeIsBuy(this.detail)
    },
    isBuyOrFree() {
      if (!(this.detail.theme_price * 1) || this.detail.isBuy) {
        return true
      }
    },
    showTxt() {
      if (this.detail.isBuy) {
        return '已购买'
      } else if (!(this.detail.theme_price * 1)) {
        return '免费'
      }
      return '立即购买'
    },
  ...
  .div(v-if="detail")
    ...
    .detail
      ...
      .price
        ...
        .method(@click="changePayType" v-if="!isBuy") 
          span 支付方式:   
          span {{payType.name}}   
          .icon( :class="payType.icon")
      .go-store-buy   
        .buy(@click="createPay") {{showTxt}}
        .store.active(@click="addBookShelft") 加入书房
