...
import { getHistoryLastAudio, isWeixin } from '@/utils/util'
export default {
  components: {
    ContentTitle,
    HomeSwiper,
  ...
  methods: {
    ...
    weixinShare() {
      if (this.$isWeixin) {
        ...
        const wxconfig = {
          title: 'X-book', // 分享标题
          desc: '数字化，一体化，专业化，生态化，助你打造自己的数字出版生态圈', // 分享描述
          link: href, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
          imgUrl: location.origin + location.pathname + '/static/img/x-booklogo.png',
        }
		...
		.content
			.section-li(v-for="item in recomList"  @click="goUrl(item)"  )
				.msg
					...
					.name.ell  {{item.name}}
				.line-single
	.column(v-for="item in column" v-if="item.book_list.length")
		.section.listen-section
			ContentTitle( :name="item.name" url="/templateList" :query="{id:item.id , name:item.name,tmp:item.tmp , type:item.type}" )
			TemplateItem( :type="item.type" :data="item.book_list" )
			.line-part
</template>
