...
var main = {
  ...
  methods: {
    ...
    change(e) {
      this.opneDetailDialog('xq',e.target.value)
    },
    query() {
    ...
    upShelf(sync) { //单个或多个上架
      let ids = []
      if (this.currentShelf) {
        ids.push(this.currentShelf.id)
      } else {
        ids = this.$getChecked({data:this.list,name:'set',value:'0',error:'请设置价格后再上架!'})
        if (!ids) return
      }
      this.services.put({id:ids,sync}).then(res => {
    ...
    getJsonLength(jsonData) { //获取json lengt
      let length = 0
    ...
    getVersion(data) {
      const dataList = data.version_list
      for(const ever in dataList){
        if(dataList[ever]!=data.id) {
          return dataList[ever]
        }
      }
    },
    delVersion(data) {//详情内删除版本
      this.$confirm("是否删除？")
        .then(_ => {
          this.services.del({id:[data.id]}).then(res => {
            this.getList()
            if(this.getJsonLength(data.version_list)<=1){
              this.detailDialog = false
            }else{
              this.opneDetailDialog('xq',this.getVersion(data))
            }
          }).catch(({payload}) => {
            for(let i in payload.messages){
              this.$message.error(payload.messages[i])
