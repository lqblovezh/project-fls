...
var _testHook = {
  ...
  // 最小长度
  equal_length: function(field, length) {
    if (!regexs.numericRegex.test(length)) return false
    return backVal(field).length === parseInt(length, 10)
  },
  // 指定字段内容是否相同
  ...
  // 指定字段内容是否相同
  same2: function(field, newField) {
    var value1 = backVal(field)
    var value2 = backVal(document.getElementById(newField).value)
    return value1 == value2
  },
    ...
    for (var i = 0, ruleLength = rules.length; i < ruleLength; i++) {
      ...
      if (failed) {
        var message = (function() {
          return (
            field.display.split('|')[i] &&
            field.display
              .split('|')
              [i].replace('{{' + field.name + '}}', field.value)
          )
        })()
