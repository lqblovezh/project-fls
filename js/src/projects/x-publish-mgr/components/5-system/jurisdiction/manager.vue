  ...
  methods: {
    ...
    showDetaile(id) {
      ...
      if (!id) {
        ...
        this.detailDate.roles = Object.keys(roleInfo.roles)[0]
        this.detailVisible = true
      }
    },
    deleteHandeler(id = []) {
          ...
          tbody(v-if="pageList.length")
            tr(v-for="item in pageList")
              ...
              td
                ...
                Icon(type="icon-preview" title="查看日志" plain size="mini" @click="viewLog(item.id)")
    ...
    el-dialog(
      ...
      el-form(:model="detailDate" :rules="detailRules" ref="detailDialog")
        ...
        el-form-item(label="角色:" label-width="120px" prop="roles")
          ...
          //-     :value="item.id"
          //-   )
          el-checkbox-group(v-model="detailDate.roles"
            :min="1"
            :max="100")
            el-checkbox(
              v-for="item in roleInfo.roleList"
